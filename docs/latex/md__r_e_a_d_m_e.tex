...
\DoxyCodeLine{Open the CMakeLists.txt file in Ot6.5.2}
\DoxyCodeLine{Insert '\(\backslash\)cosc345\(\backslash\)build' into the build directory between the parent directory and the build directory}
\DoxyCodeLine{  i.e.(...\(\backslash\)parent-\/directory\(\backslash\)cosc345\(\backslash\)build\(\backslash\)build-\/cosc345...)}
\DoxyCodeLine{Build and Run the Application in Release mode}
